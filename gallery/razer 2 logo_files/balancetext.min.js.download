...
var orphanWords = function () {
    ...
    document.querySelectorAll("orphan-text:not(.orphanfixed), .orphan-text:not(.orphanfixed)").forEach(function (elem) {
        ...
        var fontsize;
		try{
			fontsize = document.defaultView.getComputedStyle(elem).getPropertyValue("font-size");
        ...
        string_arr.forEach(function(string,i){
            string_arr[i] = string_arr[i].replace(/( *[^ ]{1,20}) ([^ ]{1,20})(\s*|\s*<\/\S+>\s*|\s*<\S+ *\S*>)$/gm, function(match,p1,p2,p3){
                let xelem = elem;
                let xcount = 0;
                while(xelem.clientWidth == 0 && xcount < 6){
                    xelem = xelem.parentElement;
                    xcount++;
                }
                let styleElem = document.defaultView.getComputedStyle(xelem);
                let width = xelem.clientWidth - parseFloat(styleElem.paddingLeft) - parseFloat(styleElem.paddingRight);
                let textpart = p1.replace(htmltag,"") + " " + p2.replace(htmltag,"") + p3.replace(htmltag,"");
                if(textpart.trim().length*fontsize*0.7 > width){
                    return match;
                }
                else{
            ...
            string_arr[i] = string_arr[i].replace(/([^ ]{1,4})(\s*|\s*<\/\S+>\s*|\s*<\S+ *\S*>)$/gm, function(match,p1,p2){
                ...
                for(let c=p1.length-1; c >= 0; c--){ 
                    if(!p1[c].match(latin) && !p1[c].match(cjk_punctuation)){
                        if(b == null){
                            b = c;
                        }
                        else if(a == null){
        ...
        string = string.replace(/Razer[™|®]? \b|RTX[™]? \b|\d (mm|m|cm|nm|um|µm|g|kg|mg|GHz|MHz|kHz|Hz|in|W|mW|A|mA|V|mV|Ω|ohm|dB|dBA|dBSPL|KB|MB|GB|TB|PB|Mbps|s|ms|nits|bit)\b|(Windows|Bluetooth|USB|HDMI|DP|Playstation|Wi-fi|Wifi|Blade|iPhone|iOS|Android|DisplayPort|Thunderbolt)[™|®]? \d/gmi, function (match) {
...
textBalancer.initialize('.balance-text, balance-text');
};
